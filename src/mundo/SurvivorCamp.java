...
public class SurvivorCamp implements Cloneable, Comparator<Puntaje> {
	...
	/**
	 * Plantilla para crear rastreros
	 */
	private Zombie plantillaRastrero;
	/**
	...
	public Zombie generarZombie(int nivel) {
		...
		if (tipoZombie == 1) {
			aGenerar = plantillaRastrero.clonar(level, zombNodoLejano);
			// aGenerar = new Rastrero(level, zombNodoLejano);
		}
		else {
			aGenerar = zombNodoLejano.clonar(level, zombNodoLejano);
	...
	public boolean acuchilla(int x, int y) {
		...
		Weapon cuchillo = WeaponFactory.getWeapon("Cuchillo");
		while (!aAcuchillar.getEstadoActual().equals(Zombie.NODO) && !seEncontro) {
			if (aAcuchillar.getEstadoActual().equals(Zombie.ATACANDO)
					&& aAcuchillar.comprobarDisparo(x, y, cuchillo.getDanio())) {
				if (aAcuchillar.getEstadoActual().equals(Zombie.MURIENDO))
					personaje.aumentarScore(40);
				seEncontro = true;
				personaje.setEnsangrentado(false);
				personaje.getCuchillo().setEstado(EstadoArma.CARGANDO);
			}
			aAcuchillar = aAcuchillar.getAtras();
		}
		if (jefe != null) {
			if (jefe.getEstadoActual().equals(Enemigo.ATACANDO) && jefe.comprobarDisparo(x, y, cuchillo.getDanio())) {
				personaje.setEnsangrentado(false);
				personaje.getCuchillo().setEstado(EstadoArma.CARGANDO);
				seEncontro = true;
				if (jefe.getEstadoActual().equals(Boss.DERROTADO)) {
