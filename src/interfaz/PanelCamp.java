...
public class PanelCamp extends JPanel implements MouseListener, KeyListener {
	...
	protected void paintComponent(Graphics arg0) {
		...
		if (matador.getCuchillo().getEstado().equals(EstadoArma.CARGANDO)) {
		...
		} else if (armaEquipada.getEstado().equals(EstadoArma.RECARGANDO)) {
			...
			arg0.drawImage(fondo, ataqueX - 33, ataqueY - 35, null);
		} else if (matador.getGranadas().getEstado().equals(EstadoArma.CARGANDO)) {
	...
	public void mousePressed(MouseEvent arg0) {
		if (principal.getEstadoPartida() == SurvivorCamp.EN_CURSO) {
			if (arg0.getButton() == MouseEvent.BUTTON1) {
				int x = arg0.getX();
				int y = arg0.getY();
				if (armaEquipada.getEstado().equals(EstadoArma.LISTA) && armaEquipada.getMunicion() > 0) {
					ultimoDisparo = arg0.getPoint();
					principal.disparar(x, y);
				} else if (y > Zombie.POS_ATAQUE && matador.getCuchillo().getEstado().equals(EstadoArma.LISTA)) {
					ultimoDisparo = arg0.getPoint();
